...
// Create brain with configuration
    bool initialize() {
        ...
        try {
            ...
            // Validate configuration values to prevent division by zero and memory issues
            NLM_VALIDATE_PARAM(neuronCount > 0, "Invalid neuron count in configuration");
            NLM_VALIDATE_PARAM(regionCount > 0, "Invalid region count in configuration");
            NLM_VALIDATE_PARAM(connectionProbability >= 0.0f && connectionProbability <= 1.0f, 
                              "Invalid connection probability in configuration");
            ...
            for (size_t i = 0; i < regionCount; ++i) {
                ...
                NLM_CHECK_NULL(region, "Failed to get region " + std::to_string(i + 1));
                ...
                if (sensoryPop) {
                    for (auto* neuron : sensoryPop->getNeurons()) {
                        NLM_CHECK_NULL(neuron, "Sensory population contains null neuron");
                        pImpl->sensoryNeurons.push_back(neuron);
                    }
                }
                if (motorPop) {
                    for (auto* neuron : motorPop->getNeurons()) {
                        NLM_CHECK_NULL(neuron, "Motor population contains null neuron");
            ...
            NLM_CHECK_NULL(pImpl->workingMemory.get(), "Working memory not initialized");
            pImpl->workingMemory->initialize(this);
            pImpl->workingMemory->setCapacity(neuronCount / 10);
    
            // Initialize episodic memory
            NLM_CHECK_NULL(pImpl->episodicMemory.get(), "Episodic memory not initialized");
            pImpl->episodicMemory->initialize(this);
            pImpl->episodicMemory->setMaxEpisodes(1000);
    
            // Initialize associative memory
            NLM_CHECK_NULL(pImpl->associativeMemory.get(), "Associative memory not initialized");
            ...
            // Initialize cognition systems
            NLM_CHECK_NULL(pImpl->planner.get(), "Planner not initialized");
            pImpl->planner->initialize(this);
            pImpl->planner->setPlanningDepth(5);
    
            NLM_CHECK_NULL(pImpl->conceptFormation.get(), "Concept formation not initialized");
            pImpl->conceptFormation->initialize(this);
    
            NLM_CHECK_NULL(pImpl->attention.get(), "Attention system not initialized");
            ...
            NLM_CHECK_NULL(pImpl->novelty.get(), "Novelty system not initialized");
            pImpl->novelty->initialize(this);
    
            NLM_CHECK_NULL(pImpl->curiosity.get(), "Curiosity system not initialized");
            pImpl->curiosity->initialize(this);
    
            // Register spike handlers for event-driven processing
            NLM_CHECK_NULL(pImpl->spikeSystem.get(), "Spike system not initialized");
            ...
            pImpl->spikeSystem->registerDelayedHandler([this](const DelayedSpikeEvent& event) {
                ...
                for (auto& region : pImpl->regions) {
                    ...
                    for (auto* neuron : neurons) {
                        NLM_CHECK_NULL(neuron, "Region contains null neuron");
            ...
            NLM_CHECK_NULL(pImpl->checkpointManager.get(), "Checkpoint manager not initialized");
            ...
            NLM_LOG_ERROR(std::string("NLM Brain initialization failed: ") + e.what());
            return false;
        } catch (const std::exception& e) {
            NLM_LOG_ERROR(std::string("Unexpected error during NLM Brain initialization: ") + e.what());
            return false;
        }
    }
...
void Brain::step(SimulationStep currentStep, Timestamp currentTime) {
    NLM_LOG_DEBUG("Brain stepping to step " + std::to_string(currentStep) + " at time " + std::to_string(currentTime));
    ...
    try {
        ...
        for (auto& region : pImpl->regions) {
            NLM_CHECK_NULL(region.get(), "Brain contains null region");
            for (auto& pop : region->getPopulations()) {
                NLM_CHECK_NULL(pop.get(), "Region contains null population");
                for (auto* neuron : pop->getNeurons()) {
                    NLM_CHECK_NULL(neuron, "Population contains null neuron");
                ...
                for (auto* neuron : pop->getNeurons()) {
                    ...
                    if (justFired) {
                        ...
                        for (Synapse* syn : incomingSynapses) {
                            NLM_CHECK_NULL(syn, "Incoming synapse is null");
                        ...
                        for (Synapse* syn : outgoingSynapses) {
                            NLM_CHECK_NULL(syn, "Outgoing synapse is null");
                            ...
                            NLM_VALIDATE_PARAM(delay > 0, "Invalid synapse delay");
                            NLM_VALIDATE_PARAM(deliveryStep > currentStep, "Invalid delivery step calculation");
        ...
        for (auto& region : pImpl->regions) {
            ...
            for (auto& syn : region->getSynapses()) {
                NLM_CHECK_NULL(syn.get(), "Region contains null synapse");
        ...
        if (currentStep % 100 == 0) {
            NLM_CHECK_NULL(pImpl->structuralPlasticity.get(), "Structural plasticity not initialized");
        ...
        if (currentStep % pImpl->replayInterval == 0 && pImpl->episodicMemory) {
            ...
            for (const auto* episode : episodesToReplay) {
                NLM_CHECK_NULL(episode, "Episode for replay is null");
        ...
        if (currentStep % 1000 == 0) {  // Update development every 1000 steps
            NLM_CHECK_NULL(pImpl->developmentSystem.get(), "Development system not initialized");
            pImpl->developmentSystem->update(this, *pImpl->rng, pImpl->timestep * 1000);
            
            // Development affects plasticity rates
            auto* sp = pImpl->structuralPlasticity.get();
            if (sp) {
        ...
        NLM_LOG_ERROR(std::string("Brain step error (code: ") + std::to_string(static_cast<int>(e.getCode())) + "): " + e.what());
        throw;  // Re-throw for higher-level handling
    } catch (const std::exception& e) {
        NLM_LOG_ERROR(std::string("Unexpected error during brain step: ") + e.what());
        throw;  // Re-throw for higher-level handling
    }
}

void Brain::receiveSensoryInput(const class SensoryInput& input) {
    NLM_LOG_DEBUG("Brain receiving sensory input");
    
    try {
        const auto& values = input.getData();
        if (values.empty()) {
            NLM_LOG_DEBUG("Empty sensory input received");
            return;
        }
        ...
        for (size_t i = 0; i < numSensory; ++i) {
            ...
            NLM_CHECK_BOUNDS(i, numSensory, "Sensory neuron index out of bounds");
        ...
        NLM_LOG_ERROR(std::string("NLM Brain receiveSensoryInput error (code: ") + std::to_string(static_cast<int>(e.getCode())) + "): " + e.what());
        throw;
    } catch (const std::exception& e) {
        NLM_LOG_ERROR(std::string("Unexpected error during brain receiveSensoryInput: ") + e.what());
        throw;
    }
}
