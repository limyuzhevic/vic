...
struct Brain::Impl {
    ...
    // Brain system updater orchestrator
    std::unique_ptr<BrainUpdaterOrchestrator> updaterOrchestrator;
        ...
        , consolidationInterval(1000)  // Consolidate every 1000 steps
    {
        ...
        // Initialize brain system updater orchestrator
        updaterOrchestrator = std::make_unique<BrainUpdaterOrchestrator>();
        ...
        NLM_LOG_INFO("Brain system updaters: " + std::to_string(pImpl->updaterOrchestrator->getUpdaterCount()));
...
// Use the orchestrator to handle Steps 1-7 (neuron, memory, neuromodulation)
    if (pImpl->updaterOrchestrator) {
        pImpl->updaterOrchestrator->update(*this, currentStep, currentTime, pImpl->timestep);
    }
