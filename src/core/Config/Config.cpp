...
bool Config::loadFromFile(const std::string& filepath) {
    // FIXED: Implement proper configuration file parsing
    // Supports simple key=value format (Phase 1 legacy) AND JSON format
    ...
    while (std::getline(file, line)) {
        ...
        // Try JSON format first (simple JSON key-value pairs)
        if (line.find('{') != std::string::npos && line.find('}') != std::string::npos) {
            // Simple JSON parsing for key-value pairs: {"key": "value", "key2": 123}
            line.erase(std::remove(line.begin(), line.end(), '{'), line.end());
            line.erase(std::remove(line.begin(), line.end(), '}'), line.end());
            
            size_t pos = 0;
            while ((pos = line.find('"', pos)) != std::string::npos) {
                size_t keyEnd = line.find('"', pos + 1);
                if (keyEnd == std::string::npos) break;
                
                std::string key = line.substr(pos + 1, keyEnd - pos - 1);
                pos = keyEnd + 1;
                ...
                if (pos < line.length() && line[pos] == ':') {
                    ...
                    if (pos < line.length() && line[pos] == '"') {
                        ...
                        if (valueEnd != std::string::npos) {
                            std::string value = line.substr(pos + 1, valueEnd - pos - 1);
                            set(key, value, ConfigSource::File);
                        }
                    } else {
                        // Try to parse numeric value
                        size_t valueEnd = line.find_first_of(",}", pos);
                        if (valueEnd != std::string::npos) {
                            std::string valueStr = line.substr(pos, valueEnd - pos);
                            // Try int
                            try {
