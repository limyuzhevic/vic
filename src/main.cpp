...
void printBanner();
void setupLogger();
bool setupConfiguration(std::shared_ptr<Config> config, int argc, char** argv);
bool initializeBrain(std::shared_ptr<Brain> brain);
void setupSimulationClock(std::shared_ptr<Config> config);
...
void displayFinalStatus(std::shared_ptr<Brain> brain);
...
int main(int argc, char** argv) {
    ...
    // Load and configure simulation parameters
    auto config = std::make_shared<Config>();
    setupConfiguration(config, argc, argv);
    
    // Setup simulation clock
    setupSimulationClock(config);
    
    // Initialize brain
    auto brain = std::make_shared<Brain>(config);
    if (!initializeBrain(brain)) {
        return 1;
    }
    ...
    // Display final status and completion message
    displayFinalStatus(brain);
