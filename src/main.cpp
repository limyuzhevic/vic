...
void applyRewardModulation(float reward, float predictedReward) {
    NLM_LOG_INFO("Applying reward modulation: reward=" + std::to_string(reward) +
                " predicted=" + std::to_string(predictedReward));
    
    // Simple reward-based plasticity implementation
    if (auto* region = brain->getRegion(RegionId(1))) {
        for (auto& syn : region->getSynapses()) {
            // Reward-modulated STDP-like learning
            float weightChange = 0.0f;
            
            // Hebbian-like potentiation
            if (reward > predictedReward) {
                weightChange += 0.001f * (reward - predictedReward);
            }
            
            // Apply weight change
            float newWeight = syn->getWeight() + weightChange;
            syn->setWeight(newWeight);
        }
    }
    
    NLM_LOG_INFO("Reward modulation completed");
}

void logSimulationStep(int step, std::shared_ptr<Brain> brain, const std::string& context) {
    if (step % 100 == 0) {
        NLM_LOG_INFO("  Step " + std::to_string(step) + " " + context +
                    " | Spikes: " + std::to_string(brain->getTotalSpikeCount()) +
                    " | Firing: " + std::to_string(brain->getFiringNeuronCount()) +
                    " | AvgRate: " + std::to_string(brain->getAverageFiringRate()));
    }
}

void printTestResults(const std::string& testName, bool success, const std::string& details = "") {
    if (success) {
        NLM_LOG_INFO("✓ " + testName + " PASSED");
        if (!details.empty()) NLM_LOG_INFO("  " + details);
    } else {
        NLM_LOG_ERROR("✗ " + testName + " FAILED");
        if (!details.empty()) NLM_LOG_ERROR("  " + details);
    }
}
