...
bool parseCommandLineArgs(int argc, char** argv, std::string& configFile, std::vector<std::string>& extraArgs) {
    bool success = true;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            printBanner();
            std::cout << "\n=== NLM Phase 2 Command Line Options ===" << std::endl;
            std::cout << "  --config=file    Load configuration from file" << std::endl;
            std::cout << "  --neuron_count=N Number of neurons (default: 500)" << std::endl;
            std::cout << "  --region_count=N  Number of neural regions (default: 1)" << std::endl;
            std::cout << "  --random_seed=N   Random seed for reproducibility (default: 42)" << std::endl;
            std::cout << "  --timestep=N      Simulation timestep in seconds (default: 0.001)" << std::endl;
            std::cout << "  --help, -h        Show this help message" << std::endl;
            std::cout << "  --verbose         Enable verbose logging" << std::endl;
            std::cout << "  --no-plasticity   Disable plasticity experiments" << std::endl;
            std::cout << "  --no-stdp         Disable STDP experiment" << std::endl;
            return false; // Exit after showing help
        } else if (arg.find("--verbose") == 0) {
            // We'll handle verbose logging in main()
        } else if (arg.find("--no-plasticity") == 0) {
            // Flag for main to skip plasticity experiment
            extraArgs.push_back(arg);
        } else if (arg.find("--no-stdp") == 0) {
            // Flag for main to skip STDP experiment
            extraArgs.push_back(arg);
...
void printConfigurationSummary(const Config& config) {
    ...
    NLM_LOG_INFO("  STDP LTP weight: " + std::to_string(config.getOr<float>("stdp_ltp_weight", 0.02f)));
    NLM_LOG_INFO("  STDP LTD weight: " + std::to_string(config.getOr<float>("stdp_ltd_weight", 0.015f)));
    NLM_LOG_INFO("  STDP time constant: " + std::to_string(config.getOr<float>("stdp_tau", 20.0f)));
    
    // Print structural plasticity parameters
    NLM_LOG_INFO("  Synaptogenesis rate: " + std::to_string(config.getOr<float>("synaptogenesis_rate", 0.0001f)));
    NLM_LOG_INFO("  Pruning rate: " + std::to_string(config.getOr<float>("pruning_rate", 0.00001f)));
    NLM_LOG_INFO("");
}

void runFullTestSuite(std::shared_ptr<Brain> brain, const std::vector<std::string>& flags) {
    bool runPlasticity = std::find(flags.begin(), flags.end(), "--no-plasticity") == flags.end();
    bool runSTDP = std::find(flags.begin(), flags.end(), "--no-stdp") == flags.end();
    ...
    if (runPlasticity) {
        ...
        NLM_LOG_INFO("=== Phase 2.1: Plasticity Learning Experiment ===");
        runPlasticityExperiment(brain);
    } else {
        NLM_LOG_INFO("=== Skipping Plasticity Experiment ===");
    }
    ...
    if (runSTDP) {
        ...
        NLM_LOG_INFO("=== Phase 2.2: STDP Verification ===");
        runStdpVerification(brain);
    } else {
        NLM_LOG_INFO("=== Skipping STDP Experiment ===");
    }
}
...
int main(int argc, char** argv) {
    ...
    if (helpRequested) {
        parseCommandLineArgs(argc, argv, configFile, extraArgs);
        return 0;
    }
    
    if (!parseCommandLineArgs(argc, argv, configFile, extraArgs)) {
        std::cout << "Argument parsing failed. Use --help for usage." << std::endl;
        return 1;
    }
    ...
    NLM_LOG_INFO("=== Phase 2.0: Basic Neural Connectivity ===");
    runBasicConnectivityTest(brain);
    
    // Run remaining experiments
    runFullTestSuite(brain, extraArgs);
