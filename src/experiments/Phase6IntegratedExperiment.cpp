...
Phase6IntegrationResult Phase6IntegratedExperiment::run(const Phase6Config& config) {
    ...
    // Create simple world
    SimpleWorld world;
    world.configure(config.neuronCount % 20 + 10, config.neuronCount % 20 + 10, 16, 16); // width, height, visionWidth, visionHeight
    ...
    for (uint64_t step = 0; step < config.maxSteps; ++step) {
        ...
        // Apply motor command to world and get reward
        ActionResult actionResult = world.applyMotorCommand(cmd, world.getSimulationTime());
        float reward = actionResult.reward; // Get reward from action result
        totalReward += reward;
