...
void PerformanceMonitor::addThreshold(const Threshold& threshold) {
    if (threshold.metric.empty()) {
        NLM_LOG_ERROR("Threshold metric cannot be empty");
        return;
    }
    if (threshold.warningLevel < 0.0 || threshold.criticalLevel < 0.0) {
        NLM_LOG_ERROR("Threshold levels cannot be negative");
        return;
    }
    ...
    for (const auto& existing : pImpl->thresholds) {
        if (existing.metric == threshold.metric) {
            NLM_LOG_WARNING("Threshold already exists for metric: " + threshold.metric);
            return;
        }
    }
        ...
        std::remove_if(pImpl->alerts.begin(), pImpl->alerts.end(),
                      [currentTime](const Alert& alert) {
                          auto alertTime = std::chrono::steady_clock::time_point(
                              std::chrono::milliseconds(static_cast<long long>(alert.timestamp)));
...
auto PerformanceMonitor::profile(const std::string& name, Func&& func, Args&&... args) -> decltype(func(std::forward<Args>(args)...)) {
    if (name.empty()) {
        NLM_LOG_ERROR("Empty function name provided to profile");
    ...
    // Update profile data with bounds checking
    auto& profileData = pImpl->profileData[name];
    if (elapsedSeconds < 0.0) elapsedSeconds = 0.0; // Clamp negative values
...
void PerformanceMonitor::toggleProfiling(const std::string& name, bool enable) {
    if (name.empty()) {
        NLM_LOG_ERROR("Empty function name provided to toggleProfiling");
        return;
    }
    ...
    if (enable) {
        ...
        if (pImpl->profileTimers.count(name)) {
            ...
            if (elapsed < 0.0) elapsed = 0.0; // Clamp negative values
...
void PerformanceMonitor::monitorThreadFunc() {
    // Main monitoring loop with proper thread safety
    while (!pImpl->stopFlag) {
        try {
            // Update metrics
            updateMetrics(pImpl->currentMetrics);
            
            // Check thresholds with thread-safe access
            checkThresholds(pImpl->currentMetrics);
            
            // Sample based on policy with atomic checks
            if (pImpl->policy == SamplingPolicy::RealTime && pImpl->samplingInterval > 0) {
                ...
                if (elapsed.count() >= pImpl->samplingInterval) {
                    ...
                    // Atomic update of lastSampleTime
                    pImpl->lastSampleTime = now;
                }
            }
            
            // Sleep with shorter interval for faster response to stopFlag
            if (pImpl->stopFlag) {
                break; // Immediate break if stopFlag is set
            }
...
void PerformanceMonitor::checkThresholds(const Metrics& metrics) {
    ...
    for (const auto& threshold : pImpl->thresholds) {
        if (threshold.metric.empty()) {
            NLM_LOG_ERROR("Threshold with empty metric name");
            continue;
        }
        ...
        if (std::isnan(currentValue) || std::isinf(currentValue)) {
            NLM_LOG_ERROR("Invalid metric value for " + threshold.metric);
            currentValue = 0.0;
        }
        
        // Check against thresholds with bounds checking
        ...
        if (isWarning || isCritical) {
            ...
            // Log the alert with proper severity
            ...
            NLM_LOG_##severity("Performance threshold exceeded: '" + threshold.name + 
                             ...
                             " (threshold: " + std::to_string(isCritical ? threshold.criticalLevel : threshold.warningLevel) + ")");
        }
    }
}
