...
public:
    ...
    // Log current experience to episodic memory
    void logExperience(const SimpleWorld& world, const ActionResult& actionResult);
    
    // Store sensory percept with reward in working memory
    void storePerceptInWorkingMemory(const SensoryPercept& percept, float reward);
    
    // Retrieve relevant experiences for current state
    void retrieveRelevantExperiences();
