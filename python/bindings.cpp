...
public:
    explicit NLMError(const std::string& msg) : std::runtime_error(msg) {}
};

class ValidationError : public NLMError {
public:
    explicit ValidationError(const std::string& msg) : NLMError("Validation error: " + msg) {}
};

class ConfigurationError : public NLMError {
public:
    explicit ConfigurationError(const std::string& msg) : NLMError("Configuration error: " + msg) {}
};

class NeuronError : public NLMError {
public:
    explicit NeuronError(const std::string& msg) : NLMError("Neuron error: " + msg) {}
};

class SynapseError : public NLMError {
public:
    explicit SynapseError(const std::string& msg) : NLMError("Synapse error: " + msg) {}
};

class WorldError : public NLMError {
public:
    explicit WorldError(const std::string& msg) : NLMError("World error: " + msg) {}
};

class ActionError : public NLMError {
public:
    explicit ActionError(const std::string& msg) : NLMError("Action error: " + msg) {}
};

// Helper function to validate neuron ID
inline NeuronId validateNeuronId(const NeuronId& id, const char* operation) {
    if (id.value == UINT64_MAX) {
        throw NeuronError(std::string("Invalid neuron ID in " + std::string(operation) + "."));
    }
    return id;
}

// Helper function to validate array bounds
template<typename T>
inline const T* validateArrayBounds(const T* data, size_t size, const char* operation) {
    if (!data && size > 0) {
        throw ValidationError(std::string("Null pointer for array in " + std::string(operation) + "."));
    }
    if (size > 1000000) {
        throw ValidationError(std::string("Array size too large in " + std::string(operation) + ". Max 1,000,000 elements."));
    }
    return data;
}

// Helper function to validate range
inline void validateRange(double value, double min, double max, const char* paramName) {
    if (value < min || value > max) {
        throw ValidationError(std::string(std::string(paramName) + " must be between " + 
                                     std::to_string(min) + " and " + std::to_string(max)));
    }
}

// Helper function to validate positive integer
inline void validatePositive(size_t value, const char* paramName) {
    if (value == 0) {
        throw ValidationError(std::string(paramName) + " must be positive"));
    }
}

// Helper function to validate non-negative
inline void validateNonNegative(double value, const char* paramName) {
    if (value < 0) {
        throw ValidationError(std::string(paramName) + " must be non-negative"));
    }
}

// Validation macro for array bounds
#define VALIDATE_ARRAY_BOUNDS(data, size, op) \
    do { \
        if ((data) && (size) > 0) { \
            validateArrayBounds(data, size, op); \
        } \
    } while(0)

// Validation macro for range checks
#define VALIDATE_RANGE(value, min, max, name) \
    do { \
        validateRange((value), (min), (max), (name)); \
    } while(0)

// Validation macro for positive check
#define VALIDATE_POSITIVE(value, name) \
    do { \
        validatePositive((value), (name)); \
    } while(0)

// Validation macro for non-negative check
#define VALIDATE_NON_NEGATIVE(value, name) \
    do { \
        validateNonNegative((value), (name)); \
    } while(0)

// Register custom exceptions
inline void registerExceptions(py::module_& m) {
    py::register_exception<std::runtime_error>(m, "RuntimeError");
    py::register_exception<NLMError>(m, "NLMError", PyExc_RuntimeError);
    py::register_exception<ValidationError>(m, "ValidationError", PyExc_ValueError);
    py::register_exception<ConfigurationError>(m, "ConfigurationError", PyExc_RuntimeError);
    py::register_exception<NeuronError>(m, "NeuronError", PyExc_RuntimeError);
    py::register_exception<SynapseError>(m, "SynapseError", PyExc_RuntimeError);
    py::register_exception<WorldError>(m, "WorldError", PyExc_RuntimeError);
    py::register_exception<ActionError>(m, "ActionError", PyExc_RuntimeError);
}
...
PYBIND11_MODULE(pynlm, m) {
    ...
    // Config class with enhanced error handling
    ...
    // Vision class with parameter validation
    py::class_<Vision, SensoryInput>(m, "Vision", R"pbdoc(Vision sensory input)pbdoc")
        ...
        .def("setData", [](Vision& self, const std::vector<float>& data) {
            VALIDATE_ARRAY_BOUNDS(data.data(), data.size(), "Vision::setData");
            VALIDATE_POSITIVE(data.size(), "Vision data size");
            self.setData(data);
        }, py::arg("data"))
    ...
    // Audio class with parameter validation
    py::class_<Audio, SensoryInput>(m, "Audio", R"pbdoc(Audio sensory input)pbdoc")
        ...
        .def("setData", [](Audio& self, const std::vector<float>& data) {
            VALIDATE_ARRAY_BOUNDS(data.data(), data.size(), "Audio::setData");
            VALIDATE_POSITIVE(data.size(), "Audio data size");
            self.setData(data);
        }, py::arg("data"))
    ...
    // SimpleWorld class with validation
    py::class_<SimpleWorld>(m, "SimpleWorld", R"pbdoc(Simple 2D world for NLM simulation)pbdoc")
        .def(py::init<>())
        .def("configure", [](SimpleWorld& self, size_t width, size_t height,
                           size_t visionWidth, size_t visionHeight) {
            VALIDATE_POSITIVE(width, "World width");
            VALIDATE_POSITIVE(height, "World height");
            VALIDATE_POSITIVE(visionWidth, "Vision width");
            VALIDATE_POSITIVE(visionHeight, "Vision height");
            if (width < visionWidth || height < visionHeight) {
                throw WorldError("World dimensions must be larger than vision dimensions");
            }
            self.configure(width, height, visionWidth, visionHeight);
    ...
    // Brain class with exception wrapping
    ...
    // AgentBrain class with validation
    ...
    m.def("createBrain", [](std::shared_ptr<Config> config) -> std::shared_ptr<Brain> {
        VALIDATE_ARRAY_BOUNDS(nullptr, 1, "createBrain"); // Just for validation demonstration
    ...
    m.def("createAgentBrain", [](std::shared_ptr<Brain> brain) -> std::shared_ptr<AgentBrain> {
        VALIDATE_ARRAY_BOUNDS(nullptr, 1, "createAgentBrain"); // Just for validation demonstration
