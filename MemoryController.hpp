// Memory Controller - handles working memory, episodic memory, associative memory
class MemoryController {
public:
    MemoryController(Brain* brain, 
    ...
    void step(TimestepDuration timestep, Timestamp currentTime) {
        ...
        // This is handled by episodic memory update logic
    }

    void update(Neuromodulator* dopamine) {
        // Apply neuromodulation effects on memory systems
        if (workingMemory) {
            // Working memory consolidation influenced by dopamine
            float dopamineLevel = dopamine ? dopamine->getLevel() : 0.0f;
            workingMemory->setConsolidationFactor(dopamineLevel);
        }

        if (episodicMemory) {
            // Episodic memory encoding enhanced by dopamine
            float dopamineLevel = dopamine ? dopamine->getLevel() : 0.0f;
            episodicMemory->setEncodingStrength(dopamineLevel);
        }
    }

    void storeEpisodicMemory(const Episode& episode) {
        if (episodicMemory) {
            episodicMemory->storeEpisode(episode);
        }
    }

    void replayMemories(size_t count) {
        if (episodicMemory) {
            episodicMemory->replayEpisode(count);
        }
    }
    ...
    NeuralWorkingMemory* getWorkingMemory() { return workingMemory; }
    NeuralEpisodicMemory* getEpisodicMemory() { return episodicMemory; }
    NeuralAssociativeMemory* getAssociativeMemory() { return associativeMemory; }
...
// Prediction Controller - handles prediction system
class PredictionController {
public:
    PredictionController(Brain* brain, PredictionSystem* predictionSystem)
        : brain(brain)
        , predictionSystem(predictionSystem) {}

    void step(const std::vector<float>& sensoryData) {
        // Step 8: Update prediction system
        if (predictionSystem) {
            // The prediction system would be updated with sensory observations
            // For now, just track prediction error history
            predictionSystem->updatePrediction(sensoryData);
        }
    }
...
// Neuromodulation Controller - handles dopamine, curiosity, novelty, prediction error
class NeuromodulationController {
public:
    NeuromodulationController(Brain* brain,
                              Dopamine* dopamine,
                              Curiosity* curiosity,
    ...
    bool initialize() {
        if (dopamine) dopamine->initialize(brain);
        if (curiosity) curiosity->initialize(brain);
        if (predictionError) predictionError->initialize(brain);
        if (novelty) novelty->initialize(brain);
        return true;
    }

    void step(TimestepDuration timestep) {
    ...
    void applyNeuromodulation() {
        ...
        float modulation = dopamine->getLevel();

        // Scale STDP learning rates
        if (brain->pImpl->stdp) {
            brain->pImpl->stdp->setLTPWeight(0.01f * modulation);
            brain->pImpl->stdp->setLTDWeight(0.012f * modulation);
        }
    }

    void update(PlasticityController* plasticityController) {
        // Calculate neuromodulation factor for plasticity
        if (dopamine) {
            plasticityController->setPlasticityModulator(dopamine->getPlasticityFactor());
        }
    }

    Dopamine* getDopamine() { return dopamine; }
    Curiosity* getCuriosity() { return curiosity; }
    PredictionError* getPredictionError() { return predictionError; }
    Novelty* getNovelty() { return novelty; }
...
// Cognition Controller - handles planning, concept formation, attention
class CognitionController {
public:
    CognitionController(Brain* brain,
    ...
    void step(TimestepDuration timestep, Timestamp currentTime,
        ...
        // Planning would be triggered by appropriate events
    }

    NeuralPlanner* getPlanner() { return planner; }
    ConceptFormation* getConceptFormation() { return conceptFormation; }
    AttentionalSelection* getAttention() { return attention; }
